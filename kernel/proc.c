...
void copy_page_state(struct page *from, struct page *to)
{
...
void fork_memory(struct proc *np)
{
  ...
  if (proc_is_not_os(p))
  {
    ...
    {
      copy_page_state(&p->pages_in_swapfile[i], &np->pages_in_swapfile[i]);
      readFromSwapFile(p, buffer, i * PGSIZE, PGSIZE);
      writeToSwapFile(np, buffer, i * PGSIZE, PGSIZE);
    }
    kfree(buffer);

    // copy the psyc papges
    for (i = 0; i < MAX_PSYC_PAGES; i++)
    {
      copy_page_state(&p->pages_in_memory[i], &np->pages_in_memory[i]);
    }
  }
}
...
#ifndef NONE // maybe this ifndef is incorrect 
  if (proc_is_not_os(p))
    removeSwapFile(p);
#endif
  acquire(&wait_lock);
